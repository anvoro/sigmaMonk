...
namespace yutokun
{
	public static class CSVParser
	{
		...
		public static List<List<string>> LoadFromPath(string path, Delimiter delimiter = Delimiter.Auto,
			Encoding encoding = null)
		{
			encoding = encoding ?? Encoding.UTF8;

			if (delimiter == Delimiter.Auto) delimiter = EstimateDelimiter(path);
		...
		/// <param name="encoding">Type of text encoding. (default UTF-8)</param>
		/// <returns>Nested list that CSV parsed.</returns>
		public static async Task<List<List<string>>> LoadFromPathAsync(string path,
		...
		private static Delimiter EstimateDelimiter(string path)
		{
			...
			if (extension.Equals(".csv", StringComparison.OrdinalIgnoreCase)) return Delimiter.Comma;

			if (extension.Equals(".tsv", StringComparison.OrdinalIgnoreCase)) return Delimiter.Tab;
