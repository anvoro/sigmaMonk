...
public class RingQTEView : MonoBehaviour
{
    ...
    [SerializeField]
    private MeshRenderer _dynamicRingRenderer;    
    [SerializeField]
    private MeshRenderer _staticRingRenderer;
    [SerializeField]
    private MeshRenderer _outerHaloRenderer;
    [SerializeField]
    private MeshRenderer _innerHaloRenderer;

    [SerializeField]
    private RingSize _staticRingSize;
    [SerializeField]
    private RingSize _dynamicRingSize;
    
    [Header("Appearance Timing")]
    [SerializeField]
    private float _apperanceScale = .4f;
    [SerializeField]
    private float _apperanceScaleDelay = .2f;
    [SerializeField]
    private float _punchScale = .6f;
    [SerializeField]
    private Vector3 _punchForce;
    [SerializeField]
    private float _punchScaleDelay = .2f;
    
    [Header("Result Timing")]
    [SerializeField]
    private float _winPunchScale = .6f;
    [SerializeField]
    private Vector3 _winPunchForce;
    [SerializeField]
    private float _losePunchPosition = .6f;
    [SerializeField]
    private Vector3 _losePunch;
    [SerializeField]
    private float _flashTime = .05f;
    [SerializeField]
    private float _resultTime = 1f;
    
    [Header("Colors")]
    [SerializeField]
    private Color _winFlashColor;
    [SerializeField]
    private Color _winColor;
    [SerializeField]
    private Color _loseFlashColor;
    [SerializeField]
    ...
    private IEnumerator PlayAppearanceAndBodyInternal(float qteTime)
    {
        ...
        float staticRingWidth = GetFloatProperty(RingProperty.Width, _staticRingMaterial);
        float dynamicRingWidth = GetFloatProperty(RingProperty.Width, _dynamicRingMaterial);
        
        float playTime = 0f;
        while (playTime < qteTime)
        {
            float t = playTime / qteTime;
            SetFloatProperty(RingProperty.Radius, _dynamicRingMaterial, Mathf.SmoothStep(_initialDynamicRadius, _targetDynamicRadius, t));
            SetFloatProperty(RingProperty.Width, _dynamicRingMaterial, Mathf.Lerp(dynamicRingWidth, staticRingWidth, t));
    ...
    {
        if (_isResultPlaying == true)
        {
            throw new Exception($"{gameObject.name} already playing ending");
        }
        ...
        Color startColor = GetColorProperty(_staticRingMaterial);
        ...
        float initialHaloRadius = haloSizeCache[_staticRingSize];
        float endHaloRadius = initialHaloRadius * 2f;
    ...
    {
        float initialHaloRadius = GetFloatProperty(RingProperty.Radius, _outerHaloMaterial);
    ...
    private IEnumerator ProcessColorLerp(float processTime, Color a, Color b, Material material)
    {
        ...
        {
            float t = currentTime / processTime;
